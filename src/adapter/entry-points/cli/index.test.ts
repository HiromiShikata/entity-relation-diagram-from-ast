...
describe('commander program', () => {
  it('should output contents', async () => {
    const output = execSync(
      'npm exec -- ts-node ./src/adapter/entry-points/cli/index.ts ./node_modules/ast-to-entity-definitions/testdata/src/domain/entities',
    ).toString();
  ...
  it('should output help', () => {
    const output = execSync(
      'npm exec -- ts-node ./src/adapter/entry-points/cli/index.ts --help',
      {
        encoding: 'utf8',
        maxBuffer: 1024 * 1000,
      },
